...
const ItemCard = ({ item, enchantLevel, version = 'v2' }) => {

    const { getMagicDmg, getPhysDmg } = useMemo(() => calculationsApi(version), [version])
    const itemPhysDmg = getPhysDmg(item, enchantLevel)
    const itemMagicDmg = getMagicDmg(item, enchantLevel)
                ...
                {item.item_magic_att_1 && <div className="col-span-2 flex flex-col text-success pt-2">
                    {[1, 2, 3, 4, 5, 6]
                        ...
                        .map(i => {
                            ...
                            return <span key={i}>{magicOpt.display_name.replace('[R]', `+${value}`)}</span>
                        })}
                </div>}
